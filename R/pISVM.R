...
svmPIDefault <- function(){
    ...
    mdply(data, function(peptide, pIExp) { pIBjell(sequence = peptide, pkSetMethod = "bjell") })
    print(data)
}

addBjell <- function(peptide){
    print(peptide)
    bjellValue = pIBjell(sequence = peptide, pkSetMethod = "bjell")
    return(bjellValue)
}

addAAIndex <- function(sequence){
